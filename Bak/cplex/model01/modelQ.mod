 ...
 * OPL 12.6.0.0 Model
 * Author: homero
 * Creation Date: 05/12/2017 at 16:44:28
 *********************************************/
/*********************************************
 * OPL 12.6.0.0 Model
 * Author: pau
...
subject to {
	...
	//----> make en=0 if zn=0 (the nurse does not work) and at most hours (for ex 24)
	forall(n in N)
		hours*z[n] >= e[n];
	...
	forall(n in N, h in 2..(hours-1)){
		...
		wa[n,h] == w[n,h+1];
		wb[n,h] == w[n,h-1];
		2*(2*(hours +1))*(1 - r[n,h]) + (2*(hours +1))*wb[n,h] - (2*(hours +1))*wa[n,h] + (2*(hours +1))*r[n,h] >= sum(h1 in 1..h) w[n,h1];
	}
