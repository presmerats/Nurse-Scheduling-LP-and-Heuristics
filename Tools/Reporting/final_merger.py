...
if __name__ == '__main__':
    ...
    folder1 = os.path.join('..','..','Results','Final','LargeSet_20180106_bak','maxiter')
    folder2 = os.path.join('..','..','Results','Final','LargeSet_20180106_bak','To_merge_maxiter')
    ...
    destination_folder = '../../Results/Final/LargeSet_20180106_bak/generation_merged'
    os.makedirs(destination_folder)
    for filename in maxiter_678:
        i1 = filename.find('-grasp')
        if i1 == -1:
            continue

        name1 = filename[:i1]
        for filename2 in maxiter_123:
            i2 = filename2.find('-grasp')
            if i2 == -1:
                continue
