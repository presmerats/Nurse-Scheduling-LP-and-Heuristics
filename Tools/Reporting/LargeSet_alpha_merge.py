...
if __name__ == '__main__':
    ...
    alpha_678910 = []
    with open('../../Results/Progress/done_alpha_last.txt','r') as fin:
        alpha_678910 = fin.readlines()

    alpha_123 = []
    with open('../../Results/Progress/done_alpha_bak.txt','r') as fin:
        alpha_123 = fin.readlines()


    # 2) intersect the list of files
    alpha_done_list= [ file[:-1] for file in alpha_123 if file in alpha_678910]

    print(len(alpha_done_list))
    #print(alpha_done_list)
    exit()
    ...
    #             possible_filename = resultdict[0].keys()
    ...
    # 6 workaround , start again with a predefined list
    destination_folder = '../../Instances/Final/LargeSet_20180106_alpha_mix'
    if not os.path.exists(destination_folder):
        os.makedirs(destination_folder)

    for file in alpha_123:
        if not file in alpha_done_list:
            filepath = file[:-1]
            print(filepath)
            print(os.path.join(destination_folder,os.path.basename(filepath)))
            print()
            
            # copy the results file
            shutil.copy(
                os.path.join(filepath),
                os.path.join(destination_folder,os.path.basename(filepath))
                )
