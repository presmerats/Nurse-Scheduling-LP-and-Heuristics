...
sortlists(filename+'summary_byint.csv', 'int_vars', results_byint )
sortlists(filename+'summary_bytime.csv', 'time', results_bytime )
sortlists(filename+'summary_byid.csv', 'id', results_bytime )
...
for filename in files:
    ...
    try:
        ...
        for elem in log:
            for k,v in elem.items():
                if  "Solution" in v and v["Solution"].find("limit")>-1:
                    results.append( 
                        { "id" : k , 
                          "time": float(v["time"]), 
                          "int_vars": int(v["int_vars"])  
...
sortlists(filename+'_unifinshed_summary_byid.csv', 'id', results_bytime )